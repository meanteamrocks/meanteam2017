 ...
 * @return bool
 */
function meanteam_twentyseventeen_categorized_blog() {

	// Get the categories.
	$all_the_cool_cats = get_transient( 'meanteam_twentyseventeen_categories' );
	if ( false === $all_the_cool_cats ) {
		...
		set_transient( 'meanteam_twentyseventeen_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so meanteam_twentyseventeen_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so meanteam_twentyseventeen_categorized_blog should return false.
		return false;
	}
}
 ...
 */
function meanteam_twentyseventeen_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function meanteam_twentyseventeen_get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
	...
	if ( ! is_numeric( $width ) ) {
		throw new Exception( __( 'Width must be an integer', 'meanteam' ) );
	}

	if ( ! is_numeric( $height ) ) {
		throw new Exception( __( 'Height must be an integer', 'meanteam' ) );
	}

	if ( ! ctype_xdigit( $background_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for background_color', 'meanteam' ) );
	}

	if ( ! ctype_xdigit( $text_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for text_color', 'meanteam' ) );
	}
	...
	$alt = apply_filters( 'meanteam_twentyseventeen_placeholder_image_alt', __( 'WebDevStudios Placeholder Image', 'meanteam' ) );
 ...
 * @return string
 */
function meanteam_twentyseventeen_get_placeholder_unsplash( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
		'category'			=> '',
