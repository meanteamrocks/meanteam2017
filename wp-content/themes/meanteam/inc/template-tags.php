...
if ( ! function_exists( 'meanteam_twentyseventeen_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function meanteam_twentyseventeen_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'meanteam' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: the post author */
			esc_html_x( 'by %s', 'post author', 'meanteam' ),
...
if ( ! function_exists( 'meanteam_twentyseventeen_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function meanteam_twentyseventeen_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'meanteam' ) );
			if ( $categories_list && meanteam_twentyseventeen_categorized_blog() ) {
				/* translators: the post category */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'meanteam' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'meanteam' ) );
			if ( $tags_list ) {
				/* translators: the post tags */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'meanteam' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'meanteam' ), esc_html__( '1 Comment', 'meanteam' ), esc_html__( '% Comments', 'meanteam' ) );
			echo '</span>';
		}
 ...
 */
function meanteam_twentyseventeen_get_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'meanteam' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'meanteam' );
	}
 ...
 */
function meanteam_twentyseventeen_get_the_title( $args = array() ) {
 ...
 * @return string The shortened excerpt.
 */
function meanteam_twentyseventeen_get_the_excerpt( $args = array() ) {
 ...
 */
function meanteam_twentyseventeen_get_post_image( $size = 'thumbnail' ) {
 ...
 */
function meanteam_twentyseventeen_get_post_image_uri( $size = 'thumbnail' ) {
 ...
 */
function meanteam_twentyseventeen_get_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'meanteam_twentyseventeen_copyright_text' );
 ...
 * @return string
 */
function meanteam_twentyseventeen_get_social_share() {
	...
	<div class="social-share">
		<h5 class="social-share-title"><?php esc_html_e( 'Share This', 'meanteam' ); ?></h5>
		<ul class="social-icons menu menu-horizontal">
			<li class="social-icon">
				<a href="<?php echo esc_url( $twitter_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php echo meanteam_twentyseventeen_get_svg( array( 'icon' => 'twitter-square', 'title' => 'Twitter', 'desc' => __( 'Share on Twitter', 'meanteam' ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Twitter', 'meanteam' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $facebook_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php echo meanteam_twentyseventeen_get_svg( array( 'icon' => 'facebook-square', 'title' => 'Facebook', 'desc' => __( 'Share on Facebook', 'meanteam' ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Facebook', 'meanteam' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $linkedin_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=475, height=505' ); return false;">
					<?php echo meanteam_twentyseventeen_get_svg( array( 'icon' => 'linkedin-square', 'title' => 'LinkedIn', 'desc' => __( 'Share on LinkedIn', 'meanteam' ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on LinkedIn', 'meanteam' ); ?></span>
				</a>
			</li>
		</ul>
	</div><!-- .social-share -->
 ...
 */
function meanteam_twentyseventeen_get_mobile_navigation_menu() {
	...
	<nav id="mobile-menu" class="mobile-nav-menu">
		<button class="close-mobile-menu"><span class="screen-reader-text"><?php echo esc_html_e( 'Close menu', 'meanteam' ); ?></span><?php echo meanteam_twentyseventeen_get_svg( array( 'icon' => 'close' ) ); // WPCS: XSS ok. ?></button>
		<?php
			wp_nav_menu( array(
 ...
 */
function meanteam_twentyseventeen_get_social_network_links() {
	...
	foreach ( $social_networks as $network ) :
		...
		$network_url = get_theme_mod( 'meanteam_twentyseventeen_' . $network . '_link' );
		...
		if ( isset( $network_url ) && ! empty( $network_url ) ) : ?>
			<li class="social-icon <?php echo esc_attr( $network ); ?>">
				<a href="<?php echo esc_url( $network_url ); ?>">
					<?php echo meanteam_twentyseventeen_get_svg( array( 'icon' => $network . '-square', 'title' => /* translators: the social network name */ sprintf( __( 'Link to %s', 'meanteam' ), ucwords( esc_html( $network ) ) ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php echo /* translators: the social network name */ sprintf( __( 'Link to %s', 'meanteam' ), ucwords( esc_html( $network ) ) ); // WPCS: XSS ok. ?></span>
				</a>
			</li><!-- .social-icon -->
		<?php endif;
	endforeach; ?>
